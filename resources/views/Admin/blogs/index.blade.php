<x-admin-layout>
    ...
    <x-slot name="subtitle">Manage your blog posts</x-slot>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            ...
            <a href="{{ route('admin.blogs.create') }}" class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 text-white font-semibold shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 transition-all duration-300 neon-glow flex items-center gap-2">
    ...
    <div class="max-w-[1600px] mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="glass-morphism rounded-2xl p-6 border-l-2 border-cyan-500">
                ...
                <p class="text-3xl font-bold text-white">{{ $blogs->total() }}</p>
            </div>
            ...
            <div class="glass-morphism rounded-2xl p-6 border-l-2 border-emerald-500">
                ...
                <p class="text-3xl font-bold text-white">{{ $blogs->where('is_published', true)->count() }}</p>
            </div>
            ...
            <div class="glass-morphism rounded-2xl p-6 border-l-2 border-amber-500">
                ...
                <p class="text-3xl font-bold text-white">{{ $blogs->where('is_published', false)->count() }}</p>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-slate-800/30 transition-colors slide-in" style="animation-delay: {{ $index * 0.05 }}s">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        @if($blog->image)
                                            <img src="{{ asset('storage/' . $blog->image) }}" alt="{{ $blog->title }}" class="w-16 h-16 rounded-lg object-cover">
                                        @else
                                        ...
                                        @endif
                                        <div>
                                            <p class="text-sm font-semibold text-white">{{ Str::limit($blog->title, 50) }}</p>
                                            <p class="text-xs text-slate-400 mt-1">{{ Str::limit($blog->excerpt, 60) }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-slate-300">{{ $blog->author->name }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.blogs.show', $blog->id) }}" class="p-2 hover:bg-cyan-500/10 text-cyan-400 rounded-lg transition-colors" title="View">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                        </a>
                                        <a href="{{ route('admin.blogs.edit', $blog->id) }}" class="p-2 hover:bg-blue-500/10 text-blue-400 rounded-lg transition-colors" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
